...
const _filterPageParams = async (input) => {
    if (input.perPageInput === '' && input.pageInput === '') {
      await service.getFilteredPlayersData(nbaAPI + "/players")
    } else if (input.perPageInput !== '' && input.pageInput === '') {
      console.log(nbaAPI + `/players?page=${input.perPageInput}`)
      await service.getFilteredPlayersData(nbaAPI + `/players?per_page=${input.perPageInput}`);
    } else if (input.perPageInput === '' && input.pageInput !== '') {
      await service.getFilteredPlayersData(nbaAPI + `/players?page=${input.pageInput}`);
    } else if (input.perPageInput && input.pageInput) {
      await service.getFilteredPlayersData(nbaAPI + `/players?per_page=${input.perPageInput}&page=${input.pageInput}`)
    }
}
...
const _filterPlayerParams = async (input) => {
  if (input.playerSearchOptions === 'search by first or last name') {
    prompt.playerSearchPrompt().then( async (data) => {
      await service.getFilteredPlayersData(nbaAPI + `/players?search=${data.playerInput}`)
    })
  } else {
    prompt.searchByPlayerIDPrompt().then( async (data) => {
      await service.logAxiosData(nbaAPI + `/players/${data.playerIDInput}`)
    })
  }
}
